...
Authenticate users with JSON Web Tokens (JWT) signed with Ed25519 (EdDSA). This method integrates your documentation with your existing authentication system and supports per-user [role-based access control](/docs/customize/access-control/overview#role-based-access-control) and personalization.
...
<Steps>
  <Step title="Select JWT in the dashboard" icon="settings" title-type="p">
    In your dashboard, go to **Settings > Access Control**, select **Private**, then choose **JWT** as the authentication method.
  </Step>
...
```
User visits docs.example.com/engineering/arch
    |
    v
Not authenticated → Redirect to your login URL
    |
    v
User authenticates in your system
    |
    v
Your backend creates JWT with accessRoles, signs with Ed25519
    |
    v
Redirect to callback URL with signed JWT
    |
    v
Documentation.AI verifies JWT, creates session cookie
    |
    v
User sees /engineering/arch (if their roles allow it)
```
