...
<ParamField path="integrations" param-type="object" required="false" deprecated="false">
  Analytics and chat integrations. Each key corresponds to a provider. See [Analytics and Chat Integrations](/integrations-and-workflows/analytics-and-chat) for all supported providers and their configuration options.
</ParamField>
...
<ParamField path="scripts" param-type="array" required="false" deprecated="false">
  ...
  <Expandable title="Script entry properties" default-open="false">
    <ParamField path="src" param-type="string" required="true" deprecated="false">
      Relative file path to a `.js` or `.mjs` file in your project. Paths must be relative — no leading `/`, `./`, or `../`. Maximum file size is 5 MB.
    </ParamField>
  </Expandable>
</ParamField>
