...
Use `integrations.cookies` to map consent categories to the cookie or localStorage values your CMP writes. Once you define categories, Documentation.ai uses them to gate integrations, custom scripts, and telemetry. Without `integrations.cookies`, integrations and scripts keep loading normally.
...
<ParamField path="integrations.cookies" param-type="object" required="false" deprecated="false">
  Cookie consent configuration. Each key is a consent category name you define, and each value tells Documentation.ai which browser value to check before that category is allowed.
  ...
  <Expandable title="Category entry properties" default-open="false">
    <ParamField path="key" param-type="string" required="true" deprecated="false">
      The cookie or localStorage key to check, such as `CookieConsent` or `OptanonConsent`.
    </ParamField>
    <ParamField path="value" param-type="string" required="true" deprecated="false">
      The substring to match in the stored value, such as `statistics:true` or `C0002:1`.
    </ParamField>
  </Expandable>
</ParamField>

<Callout kind="info" collapsed="false">
  When cookie consent is configured, analytics providers default to the `analytics` category, Clearbit defaults to `marketing`, and chat providers such as Intercom and Crisp stay `essential` unless you override `category` on the integration itself.
</Callout>

### Telemetry

Control Documentation.ai's built-in page view tracking. Telemetry is enabled by default.

- If `integrations.cookies` is not configured, telemetry starts tracking immediately.
- If `integrations.cookies` is configured, telemetry waits for the category in `integrations.telemetry.category`.
- If you do not set a category, telemetry uses `analytics` by default.
...
<ParamField path="integrations.telemetry.enabled" param-type="string" required="false" deprecated="false">
  Set to `"false"` to disable page view tracking. If you omit this field, telemetry stays enabled.
</ParamField>

<ParamField path="integrations.telemetry.category" param-type="string" required="false" deprecated="false">
  Consent category telemetry should wait for when cookie consent is active. Defaults to `"analytics"`.
</ParamField>
...
<ParamField path="scripts" param-type="array" required="false" deprecated="false">
  ...
  <Expandable title="Script entry properties" default-open="false">
    ...
    <ParamField path="category" param-type="string" required="false" deprecated="false">
      Consent category name matching a key in `integrations.cookies`. Scripts without a category remain `essential` and always load.
    </ParamField>
  </Expandable>
</ParamField>
