---
title: Quickstart
description: 'Create, customize, and publish your first Documentation.AI site in minutes.'
type: tutorial
---
## Overview

When you create a new project, Documentation.AI automatically generates a personalized, branded documentation site. This includes:

- A live URL

- Starter pages and navigation

- Your brand colors and logo fetched from your website (if available and if you choose personalization)

- AI‑powered search and assistance

This quickstart walks you through accessing your site, choosing an editing workflow, making your first update, and publishing changes.

<Callout kind="info" collapsed="false">
  You can complete this quickstart in about 15–20 minutes. No coding knowledge is required.
</Callout>

## 1. Access your live documentation site

After creating a project, your site is deployed automatically. You can find your live URL in the project dashboard under **Site URL**.

Example format:

- `https://your-project.documentationai.io`

Use this URL to preview changes and share drafts with teammates.

## 2. Choose your editing workflow

Documentation.AI supports two workflows: **Web Editor** and **Code Editor**. Both will always be in sync. You can choose which ever your are comfortable.&#x20;

<Tabs>
  <Tab title="Web Editor" icon="globe">
    Use the in-browser Web Editor to create, edit, organize, and publish content visually. This is the fastest way to publish without using version control.
  </Tab>

  <Tab title="Code Editor" icon="code">
    Use Git-backed editing with GitHub or GitLab. Edit locally in your preferred IDE and use pull requests for review. Ideal for engineering teams and version-controlled workflows.
  </Tab>
</Tabs>

## 3. Make your first edit

You will now update the auto-generated **Getting Started** page. Follow the steps for the workflow you selected.

<Steps>
  <Step title="Web Editor workflow" icon="edit-2" title-type="p">
    Open the **Getting Started** page from the sidebar in the dashboard.

    1. Click **Edit Page**.

    2. Place your cursor anywhere in the page.

    3. Type to  / see the list of components available.&#x20;

    4. Start writing and click **Publish** to push the change live.

    <Callout kind="tip" collapsed="false">
      For more detail, see the [Web Editor Overview](/write-and-publish/web-editor/overview).
    </Callout>
  </Step>

  <Step title="Code Editor workflow" icon="git-branch" title-type="p">
    1. Set up Git integration in **Settings → Integrations**.

    2. Clone your repository:

       ```bash
       git clone https://github.com/your-org/your-docs.git
       ```

    3. Edit the `getting-started` pages in your editor.

    4. Add a component such as a callout:

       ```mdx
       <Callout kind="info">
         Remember to keep your API key secret.
       </Callout>
       ```

    5. Commit and push:

       ```bash
       git add .
       git commit -m "First content update"
       git push origin main
       ```

    Your updates deploy automatically.

    <Callout kind="info" collapsed="false">
      Learn more in the [Code Editor Overview](/write-and-publish/code-editor/overview).
    </Callout>
  </Step>
</Steps>

## 4. Publish changes

Publishing behaves differently depending on your workflow.

<Columns cols="2">
  <Card title="Publish with Web Editor" href="/write-and-publish/web-editor/publish" icon="upload" horizontal="false">
    Click **Publish** in the top-right corner of the Web Editor. Changes deploy immediately.
  </Card>

  <Card title="Publish with Code Editor" href="/write-and-publish/code-editor/publish" icon="upload" horizontal="false">
    Commits to your main branch trigger automatic deployment.
  </Card>
</Columns>

## 5. Optional: connect a custom domain

If you want branded docs like `docs.yourcompany.com`, configure a custom domain.

<Expandable title="How to configure a custom domain" default-open="false">
  1. Go to **Overview → Custom Domain**.

  2. Enter your domain (for example, `docs.yourcompany.com`).

  3. Add the DNS records shown.

  4. Wait for verification and SSL provisioning.

  See full instructions: [Custom Domain Setup](/customization-and-configuration/custom-domain)
</Expandable>

## 6. Enhance your site

After publishing your first update, explore these features to extend your documentation.

<Columns cols="2">
  <Card title="Import your API schema" href="/api-documentation-and-playground/openapi-import" icon="code" cta="Import OpenAPI" horizontal="false">
    Auto-generate API reference pages from your OpenAPI spec.
  </Card>

  <Card title="Add branding" href="/customization-and-configuration/branding" icon="award" cta="Customize" horizontal="false">
    Update colors, logos, and typography.
  </Card>
</Columns>

<Columns cols="2">
  <Card title="Use more components" href="/components" icon="component" cta="Browse Components" horizontal="false">
    Insert cards, diagrams, steps, tabs, media, and more.
  </Card>

  <Card title="Optimize metadata" href="/seo-and-geo/meta-tags-setup" icon="tag" cta="Improve SEO" horizontal="false">
    Configure meta tags and search visibility.
  </Card>
</Columns>

##

