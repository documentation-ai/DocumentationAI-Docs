...
description: >-
  ...
  including dashboard settings, JSON config keys, and common issues.
---
## Overview
...
Control whether Documentation.AI generates and serves `/sitemap.xml` using either the Dashboard UI or the global SEO keys in your root `documentation.json`.
...
<Steps>
  <Step title="Open sitemap settings in the dashboard" icon="settings" title-type="p">
    - In the Dashboard, go to **Settings**.

    - Under **General**, go **SEO Settings**, **Sitemap**
  </Step>
...
Set global sitemap behavior directly in your root `documentation.json` file using the `seo` object. This mirrors the setting written by the Dashboard.
