...
function readTypeFromParagraph(src) {
  const m = src.match(/# .+?\n\n\[([^\]]+)\]/m);
  return m ? m[1] : null;
}
...
for (const file of walk(root)) {
  ...
  const expected = typeMap.get(key);
  if (!expected) continue;
  const content = fs.readFileSync(file, 'utf8');
  const found = readTypeFromParagraph(content);
  if (!found) {
    missing.push({ file: key, expected });
  } else if (found !== expected) {
    mismatches.push({ file: key, expected, found });
  }
}
...
if (missing.length || mismatches.length) {
  if (missing.length) {
    ...
    for (const m of missing) console.log(`- ${m.file} -> expected [${m.expected}]`);
  }
  if (mismatches.length) {
    console.log('Mismatched tags:');
    for (const m of mismatches) console.log(`- ${m.file} -> expected [${m.expected}] but found [${m.found}]`);
  }
  process.exitCode = 1;
} else {
  console.log('All doc-type tags match the expected structure.');
}
