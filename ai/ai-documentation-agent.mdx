...
<Callout kind="info">
  By default, the Agent uses your documentation project's context: site structure, page configuration, and Documentation.AI component syntax. When you install the GitHub Integration and connect context repositories to a project, the Agent can also read code files and recent commits from those specific repositories to inform its suggestions.
</Callout>
...
When you use the [GitHub Integration](/integrations-and-workflows/github-integration) and connect context repositories to a project, the Agent can use code files and recent commits from those repositories as context while editing your docs.
...
- *Here is the PR link that renames the `customer_id` field to `user_id`. Find all affected API docs in this project and propose updated wording.*

- *This commit adds a new `DELETE /v1/teams/{team_id}` endpoint in our context repo. Draft a new reference page that matches the style of our existing team endpoints.*

- *Use this diff to update parameter descriptions for `POST /v1/sessions` and call out the new `expires_after` field in a Callout.*

- *Review this PR that changes how rate limits work and suggest updates to the rate limits and error codes documentation.*

- *This commit deprecates the `mode` query parameter. Propose how to mark it as deprecated in our API reference and add a short migration note.*
