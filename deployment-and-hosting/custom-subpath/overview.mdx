...
A custom subpath deployment allows you to host your documentation at a specific path on your domain, such as `yoursite.com/docs` or `yoursite.com/help`, rather than on a separate subdomain like `docs.yoursite.com`.
...
1. **User visits** `yoursite.com/docs`
...
6. **Browser displays** content at `yoursite.com/docs`
...
````mermaid
graph LR
    A[User Browser] -->|yoursite.com/docs| B[Reverse Proxy]
    B -->|Strips /docs| C[Documentation Host]
    C -->|Returns Content| B
    B -->|Adds /docs back| A
...
The reverse proxy:
- Routes `/docs/*` traffic to your documentation hosting
...
<Steps>
  ...
  <Step title="Reverse Proxy Platform" icon="cloud">
...
<Columns cols={3}>
  <Card title="Vercel" icon="triangle" href="/deployment-and-hosting/custom-subpath/vercel">
    Simple JSON configuration for Vercel deployments.
  </Card>
  <Card title="Cloudflare Workers" icon="cloud" href="/deployment-and-hosting/custom-subpath/cloudflare">
    Flexible worker scripts for advanced routing.
  </Card>
  <Card title="AWS Route 53 & CloudFront" icon="cloud-cog" href="/deployment-and-hosting/custom-subpath/aws">
