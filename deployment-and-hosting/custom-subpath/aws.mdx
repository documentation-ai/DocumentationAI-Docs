...
<Callout kind="info">
  **Flexible Subpath**: Throughout this guide, we use `/docs` as an example, but you can use any subpath such as `/help`, `/guide`, `/documentation`, `/api-docs`, or `/support`. Simply replace `/docs` with your chosen path in all CloudFront behavior configurations.
</Callout>
...
<Callout kind="tip">
  Replace `/docs` with your chosen subpath in the behavior configurations. For example, if you want `/help`, create behaviors for `/help` and `/help/*` instead.
</Callout>
