...
Deploy your documentation at a custom subpath such as `yoursite.com/docs` using AWS Route 53 for DNS management and CloudFront for content delivery. This guide provides a complete enterprise-grade setup with advanced caching, security, and global distribution.

<Callout kind="info">
  Before you begin, ensure you have an AWS account with access to Route 53 and CloudFront. Your documentation should already be accessible at `[SUBDOMAIN].documentationai.io`.
</Callout>
...
| `/docs/*` | CachingOptimized | Documentation | Documentation pages |
| `/docs` | CachingOptimized | Documentation | Documentation root |
| `/dai-assets/_next/static/*` | CachingOptimized | Documentation | Static assets |
| `Default (*)` | CachingOptimized | Main Site | Landing page |

<Callout kind="alert">
  All behaviors must use the origin request policy `AllViewerExceptHostHeader` to ensure proper header forwarding.
</Callout>

## Prerequisites

<Steps>
  <Step title="AWS Account" icon="cloud">
    Active AWS account with billing enabled.
  </Step>
  
  <Step title="Domain in Route 53" icon="globe">
    Your domain managed in Route 53 (or ability to update DNS).
  </Step>
  
  <Step title="Staging URL" icon="layout">
    A staging URL for your main site (e.g., Vercel preview URL, Webflow staging).
  </Step>
  
  <Step title="Documentation Subdomain" icon="book">
...
<Steps>
  <Step title="Navigate to Origins" icon="layers">
    In your distribution, go to the **Origins** tab.
  </Step>
  
  <Step title="Find Staging URL" icon="search">
  ...
  <Step title="Create Origin" icon="plus">
...
<Steps>
  <Step title="Navigate to Behaviors" icon="route">
    Go to the **Behaviors** tab in your distribution.
  </Step>
  
  <Step title="Create Behavior" icon="plus">
    Click **Create behavior**.
  </Step>
  
  <Step title="Configure Path" icon="folder">
    Set **Path pattern** to `/.well-known/*`.
  </Step>
  
  <Step title="Set Origin" icon="server">
    Set **Origin** to your documentation origin (`[SUBDOMAIN].documentationai.io`).
  </Step>
  
  <Step title="Disable Caching" icon="x-circle">
    Set **Cache policy** to **CachingDisabled**.
  </Step>
  
  <Step title="Set Request Policy" icon="settings">
    Set **Origin request policy** to **AllViewerExceptHostHeader**.
  </Step>
  
  <Step title="Force HTTPS" icon="lock">
...
<Callout kind="info">
  If `/.well-known/*` is too broad, you can create two separate behaviors for `/.well-known/vercel/*` and `/.well-known/acme-challenge/*`.
</Callout>
...
<Steps>
  ...
  <Step title="Set Path" icon="folder">
    Set **Path pattern** to `/docs` (or your chosen subpath).
  </Step>
...
<Steps>
  ...
  <Step title="Set Wildcard Path" icon="folder">
    Set **Path pattern** to `/docs/*` (adds wildcard to match nested paths).
  </Step>
  
  <Step title="Match Previous Settings" icon="copy">
...
<Steps>
  ...
  <Step title="Set Asset Path" icon="image">
    Set **Path pattern** to `/dai-assets/_next/static/*`.
  </Step>
  
  <Step title="Configure Caching" icon="database">
...
<Steps>
  <Step title="Select Default" icon="star">
    Find the **Default (*)** behavior and click **Edit**.
  </Step>
  
  <Step title="Change Origin" icon="server">
...
<Steps>
  <Step title="Navigate to Route 53" icon="route">
    Open [Route 53](https://console.aws.amazon.com/route53/) in the AWS Console.
  </Step>
  
  <Step title="Select Hosted Zone" icon="folder">
    Navigate to **Hosted zones** and select your domain.
  </Step>
  
  <Step title="Create Record" icon="plus">
    Click **Create record**.
  </Step>
  
  <Step title="Enable Alias" icon="link">
    Toggle **Alias** to ON.
  </Step>
  
  <Step title="Route to CloudFront" icon="cloud">
    Set **Route traffic to** → **Alias to CloudFront distribution**.
  </Step>
  
  <Step title="Select Distribution" icon="select">
    Choose your CloudFront distribution from the dropdown.
  </Step>
  
  <Step title="Create Record" icon="check">
...
<Steps>
  <Step title="Test Main Domain" icon="home">
    Visit `https://yoursite.com` - should display your main site.
  </Step>
  
  <Step title="Test Documentation" icon="book">
    Visit `https://yoursite.com/docs` - should display documentation.
  </Step>
  
  <Step title="Test HTTPS" icon="lock">
    Verify the SSL certificate is valid and showing your domain.
  </Step>
  
  <Step title="Test Nested Pages" icon="folder">
    Navigate through multiple documentation pages.
  </Step>
  
  <Step title="Check Performance" icon="zap">
...
<Steps>
  <Step title="Create Invalidation" icon="refresh-cw">
    Go to **Invalidations** tab → **Create invalidation**.
  </Step>
  
  <Step title="Specify Paths" icon="folder">
    Enter paths to invalidate:
    - `/docs/*` - All documentation
    - `/docs/specific-page` - Single page
  </Step>
...
<Steps>
  <Step title="Navigate to Error Pages" icon="alert-triangle">
    Go to **Error pages** tab → **Create custom error response**.
  </Step>
  
  <Step title="Configure 404" icon="file-question">
    Set HTTP error code to **404** and customize the response.
  </Step>
  
  <Step title="Add More Errors" icon="plus">
...
<Steps>
  <Step title="Navigate to Restrictions" icon="globe">
    Go to **Restrictions** tab → **Edit**.
  </Step>
  
  <Step title="Enable Restrictions" icon="lock">
...
<Steps>
  <Step title="Edit Origin" icon="settings">
    Go to **Origins** → Select origin → **Edit**.
  </Step>
  
  <Step title="Enable Shield" icon="shield">
