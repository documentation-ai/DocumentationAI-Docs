...
Deploy your documentation at a custom subpath such as `yoursite.com/docs` using Cloudflare Workers. This guide provides a complete setup for using Cloudflare's edge computing platform to proxy documentation requests with minimal latency and maximum flexibility.
...
<Steps>
  <Step title="Create Cloudflare Worker" icon="code">
    Set up a new Worker in your Cloudflare dashboard.
  </Step>
  
  <Step title="Configure Worker Script" icon="settings">
    Add the proxying logic to route documentation requests.
  </Step>
  
  <Step title="Add Custom Domain" icon="globe">
...
```
Matches any path starting with `/docs` and proxies to your documentation.
...
<Steps>
  ...
  <Step title="Access Domains & Routes" icon="route">
  ...
  <Step title="Add www Variant" icon="globe">
...
<Steps>
  <Step title="Test Documentation Root" icon="home">
  ...
  <Step title="Check Main Site" icon="layout">
    Verify your main site at `yoursite.com` still loads correctly.
  </Step>
  
  <Step title="Verify Assets" icon="image">
