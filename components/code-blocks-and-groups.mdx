...
You can enable wrapping in several equivalent ways on a fenced block: `wrap`, `wrap="true"`, `wrap=true`. All of these cause long lines to wrap to the container width using `white-space: pre-wrap` and `word-break: break-word`, with vertical overflow allowed (`overflow-y: auto` only).
