...
`CollectionList` renders the **direct children** of a navigation node in one of four layouts. Instead of manually listing links to child pages, you point the component at a navigation node and it automatically displays the immediate children in the layout you choose. It does not recurse through nested groups or pages deeper in the tree. If you need the full hierarchy, use `CollectionContent` instead.
...
<Tabs>
  ...
  <Tab title="Links" icon="link">
    Displays children as plain bulleted links with minimal styling.
...
A minimal example with only the required attribute. If you omit `layout`, `CollectionList` uses the `cards` layout by default:

```jsx
<CollectionList node="tabs:Guides" />
```

A full example showing the optional attributes for the `cards` layout:
...
<Callout kind="info" collapsed="false">
  `cols` and `card-variant` only apply to the `cards` layout. `default-open` only applies to the `accordion` layout and is written as `default-open="true"` or `default-open="false"` in MDX. Attributes that do not match the current layout are ignored.
</Callout>
...
<ParamField path="layout" param-type="string" required="false" deprecated="false">
  Display layout. Accepted values: `"cards"`, `"accordion"`, `"list"`, `"links"`. Defaults to `"cards"`, so omitting `layout` renders direct children as cards.
</ParamField>

<ParamField path="cols" param-type="number" required="false" deprecated="false">
  Number of grid columns for the `cards` layout. Write it as `cols={2}` in MDX. Accepted values: `1`, `2`, `3`, `4`. Defaults to `2`. Ignored for other layouts.
</ParamField>
...
<ParamField path="default-open" param-type="boolean" required="false" deprecated="false">
  Whether the accordion starts expanded. Write it as `default-open="true"` or `default-open="false"` in MDX. Only applies to the `accordion` layout. Defaults to `true`.
</ParamField>
