...
<Tabs>
  ...
  <Tab title="Accordion" icon="chevron-down">
    Displays one collapsible section for the selected node. When expanded, it shows the node's direct children as a stacked list of links.

    ```jsx
    <CollectionList node="tabs:Guides" layout="accordion" default-open={true} />
    ```

    Use accordion when you want a compact overview that readers can expand or collapse without taking over the page.
  </Tab>
  ...
  <Tab title="Links" icon="link">
    Displays children as a simple vertical list of text links with minimal styling.
...
<ParamField path="default-open" param-type="boolean" required="false" deprecated="false">
  Whether the selected node's accordion section starts expanded. Only applies to the `accordion` layout. Defaults to `true`.
</ParamField>
