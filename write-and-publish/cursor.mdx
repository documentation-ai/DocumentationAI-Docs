...
<Steps>
  ...
  <Step title="Add MCP server to Cursor" icon="settings">
    ...
    **File location:** `~/.cursor/mcp.json`
    ...
    This connects to Documentation.AI's official MCP server. If you're using a custom domain, replace the URL with your own MCP server endpoint (typically `https://docs.yourdomain.com/_mcp` or `https://yourdomain.com/docs/_mcp`).

    <Callout kind="tip">
    If you already have other MCP servers configured, add the `documentation-ai` entry to your existing `mcpServers` object.
    </Callout>

    <Callout kind="info">
    The MCP server uses Server-Sent Events (SSE) for real-time communication with Cursor's AI features.
    </Callout>
  </Step>
  
  <Step title="Restart Cursor" icon="rotate-cw">
    Restart Cursor to load the new MCP server configuration.
    
    After restarting, the MCP server will be available to Cursor's AI features in Composer.
    
    <Callout kind="alert">
    MCP tools are only available to the Agent in Composer, not in the inline chat.
    </Callout>
  </Step>
  
  <Step title="Test the connection" icon="check">
    Open Composer in Cursor and ask a question about your documentation to verify the MCP server is connected and working.
    
    Example prompts to test:
    - "Search my documentation for authentication setup"
    - "What does my documentation say about API configuration?"
